...
import { getReportSales, getReportSalesDownload } from "../../services/reporte";
...
const ReportSalesPage = () => {
  ...
  const fetchData = async (pageNumber = 1, pageSize = 10) => {
    try {
      ...
      const { status, data, totalPages: tp } = await getReportSales(params);
      switch (status) {
        case 200:
          setReportData(data);
          if (totalPages != tp) {
  ...
  const handleDownloadSales = async () => {
    ...
    try {
      ...
      const { status, data } = await getReportSalesDownload(params);
      switch (status) {
        case 200:
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-end h-full">
              <button
                ...
                className="flex items-center justify-center gap-2 bg-cyan-700 text-white w-full h-11 rounded-lg hover:bg-cyan-600 transition duration-300 shadow-md"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
